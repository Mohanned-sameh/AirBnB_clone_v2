...
def do_deploy(archive_path):
    ...
    if archive_path:
        ...
        run("mkdir -p /data/web_static/releases/{}/".format(archive_wo_ext_ver))
        run(
            "tar -zxf /tmp/{} -C /data/web_static/releases/{}/".format(
                archive_wo_ver, archive_wo_ext_ver
            )
        )
        ...
        /data/web_static/releases/{}/".format(
                archive_wo_ext_ver, archive_wo_ext_ver
            )
        )
        run("rm -rf /data/web_static/releases/{}/web_static".format(archive_wo_ext_ver))
...
def do_pack():
    try:
        ...
        local(
            "tar -zcvf versions/web_static_$(date +%Y%m%d%H%M%S).tgz\
        web_static"
        )
